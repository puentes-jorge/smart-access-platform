...
export function AppNav() { return <aside className="sidebar"><div className="brand"><span className="brand-mark"><icons.shield size={20}/></span><span>Smart Access</span></div><div className="nav-label">Administración</div><nav>{nav.map(([label, href, Icon]) => <Link className="nav-link" href={href} key={href}><Icon size={18}/><span>{label}</span></Link>)}</nav><div className="sidebar-foot"><form action="/api/auth/logout" method="post"><button className="secondary" style={{width:"100%"}}>Cerrar sesión</button></form></div></aside>; }
